...
const addImportantMsg_1 = require("./addImportantMsg");
...
function getMsg(userId, accessToken, msgId, callback) {
    ...
    rp(options).then(body => {
        ...
        header.forEach(function (val) {
            if (val.name === "From") {
                important = filter_1.filterMsg(userId, val.value, notification.title).then(resolve => {
                    addImportantMsg_1.addImportantMsg(userId, msgId, true);
                ...
                if (JSON.stringify(message.labelsAdded) !== undefined) {
                    ...
                    labelAdded.forEach(label => {
                        if (label.labelIds[0] === 'TRASH') {
                            addImportantMsg_1.addImportantMsg(userId, label.message.id, null);
                        }
                    });
                }
            });
